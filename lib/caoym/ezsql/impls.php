...
class ValuesImpl
{
    static public function values($context, $values){
        ...
        $stubs = [];
        foreach ($values as $v){
            if(is_a($v, 'caoym\\ezsql\\Native')){//直接拼接sql，不需要转义
                $stubs[]=$v->get();
            }else{
                $stubs[]='?';
                $params[] = $v;
...
class UpdateSetImpl
{
    public function set($context, $column, $value){
        ...
        if(is_a($value, 'caoym\\ezsql\\Native')){
        ...
        foreach ($args as $k => $v){
            if(is_array($v)){
                ...
                if($op == 'IN' || $op == 'NOT IN'){
                    ...
                    $cond = "$k BETWEEN";
                    if(is_a($var[0], 'caoym\\ezsql\\Native')){
                        $cond = "$cond ".strval($var[0]);
                    }else{
                        $cond = "$cond ?";
                        $params[] = $var[0];
                    }
                    if(is_a($var[1], 'caoym\\ezsql\\Native')){
        ...
        if($expr){
            if($args){
                ...
                foreach ($args as $k =>$arg){
                    if(is_array($arg) || is_a($arg, 'caoym\\ezsql\\Native')){
                        if(!$cutted){
