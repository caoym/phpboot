...
class DBTest extends TestCase
{
    ...
    public function testSelect0()
    {
        ...
        (new DB($this->db))->select('c=1')->get();  
        
        $this->db->setExpected('SELECT *');
        (new DB($this->db))->select()->get();
    }
    public function testSelect1()
    {
        // SELECT col FROM tab
        $this->db->setExpected('SELECT `col` FROM `tab`');
        (new DB($this->db))->select('col')->from('tab')->get();
    }
    ...
    public function testSelect5()
    {
        ...
        (new DB($this->db))->select('col')->from('tab')->leftJoin('tab1')->on('tab.id=tab1.id')->get();
    }
    public function testSelect6()
    {
        ...
        (new DB($this->db))->select('col')->from('tab')->rightJoin('tab1')->on('tab.id=tab1.id')->get();
    }
    public function testSelect7()
    {
        ...
        (new DB($this->db))->select('col')->from('tab')->innerJoin('tab1')->on('tab.id=tab1.id')->get();
    }
    public function testSelect8()
    {
        ...
        (new DB($this->db))->select('col')->from('tab')->join('tab1')->on('tab.id=tab1.id')->get();
    }
    public function testSelect9()
    {
        ...
        (new DB($this->db))->select('col')->from('tab')->join('tab1')->on('tab.id=tab1.id')->where('col=1')->get();
    }
    public function testSelect10()
    {
        // SELECT * FROM tab GROUP BY col
        $this->db->setExpected('SELECT * FROM `tab` GROUP BY col');
        (new DB($this->db))->select('*')->from('tab')->groupBy('col')->get();
    }
    public function testSelect11()
    {
        ...
        (new DB($this->db))->select('SUM(col)')->from('tab')->groupBy('col1')->having('SUM(col)>?',0)->get();  
    }
    ...
    {     
        ...
        (new DB($this->db))->select('SUM(col)')->from('tab')->where('col=?',1)->groupBy('col1')->having('SUM(col)>?',0)->get();
    ...
    public function testSelect13()
    {
        ...
        (new DB($this->db))->select('*')->from('tab')->orderBy('col')->get();
        (new DB($this->db))->select('*')->from('tab')->orderByArgs(['col'])->get();
    }
    public function testSelect14()
    {
        // SELECT * FROM tab ORDER BY col ASC
        $this->db->setExpected('SELECT * FROM `tab` ORDER BY col ASC');
        (new DB($this->db))->select('*')->from('tab')->orderBy('col', DB::ORDER_BY_ASC)->get();
        (new DB($this->db))->select('*')->from('tab')->orderByArgs(['col'=>DB::ORDER_BY_ASC])->get();
    }
    ...
    public function testUpdate0()
    {
        ...
        (new DB($this->db))->update('tab')->setArgs(['a'=>1,'b'=>'2','c'=>DB::raw('now()')])->exec();
        (new DB($this->db))->update('tab')->set('a',1)->set('b', '2')->set('c', DB::raw('now()'))->exec();
        (new DB($this->db))->update('tab')->set('a',1)->setArgs(['b'=>'2','c'=>DB::raw('now()')])->exec();
        (new DB($this->db))->update('tab')->setArgs(['a'=>1,'b'=>'2'])->set('c',DB::raw('now()'))->exec();;
    }
    public function testUpdate1()
    {
        //UPDATE `tab` SET a=1 WHERE b='2'
        $this->db->setExpected('UPDATE `tab` SET a=? WHERE b=?', 1,'2');
        (new DB($this->db))->update('tab')->set('a', 1)->where('b=?',2)->exec();;
    }
    ...
    public function testUpdate3()
    {
        ...
        (new DB($this->db))->update('tab')->set('a', 1)->where('b=?',2)->orderBy('c')->exec();;
    }
